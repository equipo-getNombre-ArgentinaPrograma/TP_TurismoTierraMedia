...
/*
	public static ArrayList<Attraction> ofAttractions() {
		...
		for (String[] tmp : Reader.get("entrada", "atracciones")) {
			try {
				attractions.add(new Attraction(attractions.size() + 1, tmp[0], Double.parseDouble(tmp[1]),
...
// lista
	public static ArrayList<User> ofUsers() {
		...
		for (String[] tmp : Reader.get("entrada", "usuarios")) {
			try {
				users.add(new User(users.size() + 1, tmp[0], Double.parseDouble(tmp[1]), Double.parseDouble(tmp[2]),
						(tmp[3])));
			} catch (NumberFormatException e) {
		...
		for (String[] tmp : Reader.get("entrada", "promociones")) {
			try {
				if (tmp[0].equals("Porcentual"))
					promo = new PorcentualProm(promos.size() + 1, atracciones, tmp[1], tmp[2], tmp[3],
							Double.parseDouble(tmp[4]));
				else if (tmp[0].equals("Absoluta"))
					promo = new AbsoluteProm(promos.size() + 1, atracciones, tmp[1], tmp[2], tmp[3],
							Double.parseDouble(tmp[4]));
				else if (tmp[0].equals("AxB"))
					promo = new AxBProm(promos.size() + 1, atracciones, tmp[1], tmp[2], tmp[3], tmp[4]);
			} catch (NumberFormatException e) {
